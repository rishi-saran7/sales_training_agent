            ...
            {data?.conversationMetrics && (
              <>
                <section>
                  ...
                  >
                    {[
                      { label: "Talk Ratio", value: `${(data.conversationMetrics.avg_talk_ratio * 100).toFixed(0)}%`, sub: "Trainee speaking share" },
                      { label: "Engagement", value: `${data.conversationMetrics.avg_engagement_score}/10`, sub: "Composite score" },
                      { label: "Questions Asked", value: `${data.conversationMetrics.avg_user_questions}`, sub: "Avg per session" },
                      { label: "Filler Rate", value: `${data.conversationMetrics.avg_filler_word_rate}%`, sub: "Of total words" },
                      { label: "Interruptions", value: `${data.conversationMetrics.avg_interruption_count}`, sub: "Avg per session" },
                      { label: "Avg Turn Length", value: `${data.conversationMetrics.avg_turn_length} words`, sub: "Per trainee turn" },
                      { label: "Speaking Pace", value: `${data.conversationMetrics.avg_words_per_minute} wpm`, sub: "Words per minute" },
                      { label: "Response Latency", value: data.conversationMetrics.avg_response_latency_ms != null ? `${(data.conversationMetrics.avg_response_latency_ms / 1000).toFixed(1)}s` : "N/A", sub: "Avg delay" },
                    ].map((item) => (
                      <div
                        key={item.label}
                        style={{
                ...
                >
                  ...
                  <div style={{ width: "100%", height: "220px" }}>
                    <ResponsiveContainer>
                      <BarChart
                        data={[
                          { topic: "Objections", pct: data.conversationMetrics.customer_objection_pct },
                          { topic: "Pricing", pct: data.conversationMetrics.pricing_session_pct },
                          { topic: "Competitors", pct: data.conversationMetrics.competitor_session_pct },
                          { topic: "Closing", pct: data.conversationMetrics.closing_session_pct },
                        ]}
                        layout="vertical"
                        margin={{ left: 24 }}
                      >
                ...
                >
                  ...
                  <div style={{ width: "100%", height: "280px" }}>
                    <ResponsiveContainer>
                      <RadarChart
                        data={[
                          { metric: "Engagement", value: data.conversationMetrics.avg_engagement_score },
                          { metric: "Talk Balance", value: Math.min(10, (1 - Math.abs(data.conversationMetrics.avg_talk_ratio - 0.5) * 4) * 10) },
                          { metric: "Questions", value: Math.min(10, data.conversationMetrics.avg_user_questions * 2) },
                          { metric: "Rapport", value: Math.min(10, data.conversationMetrics.avg_rapport_phrases * 2) },
                          { metric: "Fluency", value: Math.max(0, 10 - data.conversationMetrics.avg_filler_word_rate) },
                        ]}
                        outerRadius={90}
                      >
                        <PolarGrid stroke="#1f2a44" />
